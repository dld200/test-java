...
public interface UIElementParser {

    UiElement parse(String xml);

    UiElement buildTree(org.w3c.dom.Element node, UiElement parent);

    default void clean(UiElement element) {
    ...
    default UiElement parseAndClean(String xml) {
        UiElement rawTree = parse(xml);
        clean(rawTree);
        return rawTree;
    }

    default void printTree(UiElement node, int depth) {
        if (node == null) return;
